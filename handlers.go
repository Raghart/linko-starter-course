...
func (s *server) handlerShortenLink(w http.ResponseWriter, r *http.Request) {
	...
	if longURL == "" {
		httpError(r.Context(), w, http.StatusBadRequest, "missing url parameter",
			errors.New("missing url parameter"))
		return
	}
	u, err := url.Parse(longURL)
	if err != nil || u.Scheme == "" || u.Host == "" {
		httpError(r.Context(), w, http.StatusBadRequest,
			"invalid url: must include scheme (http/https) and host", err)
		return
	}
	...
	if err != nil {
		httpError(r.Context(), w, http.StatusInternalServerError, "failed to shorten url", err)
		return
	}
...
func (s *server) handlerRedirect(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		if errors.Is(err, store.ErrNotFound) {
			httpError(r.Context(), w, http.StatusNotFound, "not found", err)
		} else {
